...
def render_data_manager():
    ...
    if crud_mode == "➕ Add New Song":
        with st.form("dm_add_song_form"):
            ...
            if submitted:
                if not s_title or not s_artist:
                    ...
                    st.success(f"Song '{s_title}' successfully created with ID: {new_song_id}!")

    elif crud_mode == "✏️ Edit Existing Song":
        search_edit = st.text_input("Find Song to Edit", placeholder="Enter song title...", key="dm_search_edit")
        if search_edit:
            matches = song_repo.search(query=search_edit, limit=5)
            if matches:
                ...
                with st.form("dm_edit_form"):
                    ...
                    e_pop = st.slider("Popularity", 0, 100, song_choice["popularity"])
                    e_genre = st.text_input("Genre", value=song_choice["genre"])
                    
                    e_save = st.form_submit_button("Update Song")
                    if e_save:
                        song_repo.update_song(song_choice["id"], {
                            "title": e_title,
        ...
        del_search = st.text_input("Find Song to Delete", placeholder="Enter song title...", key="dm_del_search")
        if del_search:
            del_matches = song_repo.search(query=del_search, limit=5)
            if del_matches:
                ...
                if st.button("Confirm Delete", type="primary", key="dm_del_confirm"):
                    ...
                    st.success(f"Deleted '{del_choice['title']}' from database.")
                    st.rerun()
